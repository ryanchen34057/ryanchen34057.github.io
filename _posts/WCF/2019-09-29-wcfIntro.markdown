...
因為服務對外界並不透明，每個WCF服務基本上會暴露**Metadata**來說明關於服務的功能以及與服務溝通的方法。**Metadata**通常以標準化的[Web Services Description Language(WSDL)](https://zh.wikipedia.org/wiki/WSDL)的語言來發佈。
...
### 位置 Address
在WCF裡，每個服務都有一個唯一的位置。提供了兩個重要的資訊:
...
```
#### 注意點
* 實作服務的類別不允許使用有待參數的建構子，因為WCF只會使用預設的建構子。
